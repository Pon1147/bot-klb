// ===== POPUP CONFIG — Quản lý codes qua chrome.storage =====
...
(() => {
  ...
  // Codes mặc định (sync từ content.js)
  const DEFAULT_CODES = `DFSL9304
DFSL6257
GARENADFNY2501C158
PWC260419S65
PWC260419S21
...
DFISPACECITYISG
DFISPACECITYMDB
DFISPACECITYECMO`.trim();

  // ===== LOAD STORED CODES ON START =====
  chrome.storage.local.get(['redeem_codes'], (data) => {
    if (data.redeem_codes && data.redeem_codes.length > 0) {
      codesInput.value = data.redeem_codes.join('\n');
    } else {
      codesInput.value = DEFAULT_CODES;
    }
  });
  ...
  btnReset.addEventListener('click', () => {
    ...
    codesInput.value = DEFAULT_CODES;
    chrome.storage.local.set({ redeem_codes: DEFAULT_CODES.split('\n').map((c) => c.trim()).filter((c) => c) }, () => {
